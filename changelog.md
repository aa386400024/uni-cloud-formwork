...
- 新增支持看激励视频广告签到
## 1.1.22（2021-11-10）
删除`common/js/openApp.js`中可选链操作符，解决vue3版本在hbuilderX内置浏览器不兼容的问题
## 1.1.21（2021-11-10）
...
## 1.0.44（2021-08-05）
解决首页为非nvue页面时白屏的问题。
- 注意：本次在`common/js/appInit.js`中修改了路由拦截的逻辑，是个兼容方案；当首页为非nvue页面，路由拦截器逻辑会在加载首页时执行。接下来新版本的hx编译的uni-app项目无论首页是否为nvue都不走拦截器，保持各端逻辑一致。
## 1.0.43（2021-08-02）
